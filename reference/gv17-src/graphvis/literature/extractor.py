...
#   5. operational parameters (voltage, pH, temperature, HRT, COD, VFA, …)
...
from graphvis.core.paths import LITERATURE_DIR
from graphvis.core.paths import LITERATURE_DATASET_DIR as _LITERATURE_DATASET_PATH
# Extracted experimental tables are written into the dedicated
# literature_dataset/ folder so they are clearly separated from the source
# documents and can be re-registered as ordinary GraphVis datasets.
LITERATURE_DATASET_DIR = str(_LITERATURE_DATASET_PATH)
...
# ----------------------------------------------------------- parameters
_PARAM_PATTERNS = {
    "applied_voltage": (rf"(?:applied\s+(?:voltage|potential)|E\s*app|cell\s+voltage)[^0-9\-]{{0,40}}({_NUM})\s*(m?V)\b", "V"),
    "pH": (rf"\bpH\s*(?:of|=|:|was|at)?\s*({_NUM})", ""),
    "temperature": (rf"({_NUM})\s*(?:°|º|deg)\s*C\b", "°C"),
    "HRT": (rf"(?:HRT|hydraulic\s+retention\s+time)[^0-9]{{0,30}}({_NUM})\s*(h|hr|hours?|d|days?)\b", ""),
    "influent_sCOD": (rf"(?:sCOD|soluble\s+COD|COD)[^0-9]{{0,40}}({_NUM})\s*(g|mg)\s*/?\s*L\b", ""),
    "VFA": (rf"(?:VFA|volatile\s+fatty\s+acids?)[^0-9]{{0,40}}({_NUM})\s*(g|mg)\s*/?\s*L\b", ""),
    "current_density": (rf"current\s+density[^0-9]{{0,30}}({_NUM})\s*(A\s*/?\s*m[²2]|mA\s*/?\s*cm[²2]|A\s*m-2)", ""),
    "H2_yield": (rf"(?:hydrogen|H2|H₂)\s+yield[^0-9]{{0,40}}({_NUM})\s*(mol\s*H2?\s*/?\s*mol\s*\w+|mL\s*/?\s*g\s*\w*|L\s*/?\s*g\s*\w*)", ""),
    "VHPR": (rf"(?:VHPR|volumetric\s+hydrogen\s+production\s+rate|hydrogen\s+production\s+rate)[^0-9]{{0,40}}({_NUM})\s*(m[³3]\s*H?2?\s*/?\s*m[³3]\s*/?\s*d(?:ay)?|L\s*/?\s*L\s*/?\s*d(?:ay)?|mL\s*/?\s*L\s*/?\s*(?:h|d))", ""),
    "coulombic_efficiency": (rf"coulombic\s+efficiency[^0-9]{{0,30}}({_NUM})\s*%", "%"),
    "anode_area": (rf"(?:anode|electrode)\s+(?:surface\s+)?area[^0-9]{{0,30}}({_NUM})\s*(cm[²2]|m[²2])", ""),
}
...
def extract_literature(path: str, extract_dataset: bool = True, ocr: bool = True,
    ...
    try:
        ...
        if budget_expired():
            raise TimeoutError("literature time budget reached")
        from graphvis.literature.semantics import analyse_literature_context
        context = analyse_literature_context(text, [d.df for d in ext.datasets])
        ext.semantic_context = context.to_dict()
        semantic_hint_map = {
            "Gompertz H₂ Kinetics": "gompertz", "Polarisation & Power Curve": "polarisation",
