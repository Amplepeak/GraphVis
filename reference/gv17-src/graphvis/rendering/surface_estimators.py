"""Scientific surface interpolation backends for GraphVis.

The renderer keeps physical plotting coordinates separate from the metric used
by interpolation.  In particular, an X or Y axis configured as Log10 is first
mapped to log10-space and then normalised to [0, 1].  Delaunay, IDW, RBF,
kriging, MLS, LOESS and natural-neighbour distances therefore operate in the
same visual metric that the user sees on a logarithmic axis without warping
the physical mesh returned to Matplotlib.

The module is intentionally Qt-free so it can run inside GraphVis worker
threads and be regression-tested headlessly.
"""
...
def _idw(points: np.ndarray, values: np.ndarray, queries: np.ndarray, *, power: float, neighbors: int,
    ...
    # Hybrid-architecture seam: use the optional compiled Rust kernel when the
    # graphvis_native wheel is installed; identical results guaranteed.
    try:
        from graphvis.core.native_accel import get_kernel
        native = get_kernel("idw", None)
        if native is not None:
            return np.asarray(native(np.ascontiguousarray(points, dtype=float),
                                     np.ascontiguousarray(values, dtype=float),
                                     np.ascontiguousarray(queries, dtype=float),
                                     float(max(power, 0.05)), int(k)), dtype=float)
    except Exception:
        pass
...
def _rbf(points: np.ndarray, values: np.ndarray, queries: np.ndarray, *, kernel: str,
    ...
    if kernel in {"multiquadric", "gaussian", "inverse_multiquadric", "inverse_quadratic"}:
        tree = tree or cKDTree(points)
        nn = tree.query(points, k=min(2, len(points)), workers=1)[0]
        nearest = nn[:, -1] if nn.ndim == 2 else nn
        nearest = nearest[np.isfinite(nearest) & (nearest > 0)]
        kwargs["epsilon"] = max(float(np.nanmedian(nearest)) if nearest.size else 0.1, 1e-4)
