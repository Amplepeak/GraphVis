...
_RESPONSE_TOKENS = (
    "response", "yield", "rate", "vhpr", "power", "efficiency", "conversion", "removal", "activity",
    "current", "voltage", "density", "concentration", "scod", "vfa", "h2", "temperature", "pressure",
)
_PARAMETER_TOKENS = (
    "lhs_", "input", "dose", "flow", "area", "voltage", "current", "temperature", "pressure", "ph",
    "concentration", "loading", "speed", "rate", "frequency", "time", "potential",
)
...
def scan_dataset(dataset: Any, literature: Iterable[Any] | None = None, *, progress=None, cancelled=None,
    ...
    def legacy_scale_from_axes(scales: dict[str, str]) -> str:
        xlog, ylog = scales.get("x") == "Log10", scales.get("y") == "Log10"
        if xlog and ylog: return "Logarithmic Scale"
        if xlog: return "Semi-Log X"
        if ylog: return "Semi-Log Y"
        return "Linear Scale"
    ...
    for rec in recs:
        ...
        diag["recommended_scale"] = legacy_scale_from_axes(axis_scales)  # old project compatibility
...
def cache_path(cache_dir: str | Path, dataset: Any, literature_fp: str | None = None) -> Path:
    ...
    return root / f"{dfp}.json"  # legacy/no-context path
