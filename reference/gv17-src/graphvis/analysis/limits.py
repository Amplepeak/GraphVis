...
from graphvis.analysis.electro import pareto_frontier
...
def confidence_envelope(x, y, level=90.0, bins=40, smooth_sigma=1.0) -> dict:
...
def pareto_bounds(x, y, maximise_y=True) -> dict:
    """Upper (Pareto) frontier and lower (anti-Pareto) frontier of a cloud."""
    x, y = _clean(x, y)
    if x.size < 5:
        return {"found": False}
    upper = pareto_frontier(x, y, maximise_y=maximise_y, n_boot=0)
    lower = pareto_frontier(x, y, maximise_y=not maximise_y, n_boot=0)
...
def analyse_limits(x, y, options: dict | None = None) -> LimitReport:
    ...
    if opts.get("confidence"):
        c = confidence_envelope(x, y, level=float(opts.get("level", 90.0)))
        if c.get("found"):
