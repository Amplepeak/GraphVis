...
impl DatasetStore {
    pub fn insert(&self, ds: ArrowDataset) {
        self.inner.write().insert(ds.meta.id, ds);
    }
    pub fn get(&self, id: Uuid) -> Option<ArrowDataset> {
        self.inner.read().get(&id).cloned()
    }
    pub fn all(&self) -> Vec<ArrowDataset> {
    ...
    pub async fn import_csv(
        ...
        let df = ctx
            .read_csv(
                ...
                CsvReadOptions::new().delimiter(delimiter),
            )
            .await?;
    ...
    fn register_batches(
        ...
        let first = batches
            .first()
            .ok_or_else(|| anyhow!("dataset contains no record batches"))?;
...
impl QueryEngine {
    ...
    pub async fn sql_to_ipc(
        ...
        while let Some(batch) = stream.next().await {
            let batch = batch?;
            if writer.is_none() {
                writer = Some(FileWriter::try_new(
...
pub fn numeric_column(ds: &ArrowDataset, column: &str) -> Result<Vec<f64>> {
    ...
    for batch in ds.batches.iter() {
        ...
        let values = cast
            ...
            .context("cast to Float64 failed")?;
