...
impl GraphVisFlightService {
    pub fn insert(&self, name: impl Into<String>, batches: Vec<RecordBatch>) {
        self.datasets.write().insert(name.into(), batches);
    }
    fn key_from_descriptor(d: &FlightDescriptor) -> String {
        if !d.path.is_empty() {
            d.path.join("/")
        } else {
            String::from_utf8_lossy(&d.cmd).to_string()
        }
    }
}
...
impl FlightService for GraphVisFlightService {
    ...
    async fn list_flights(
        ...
        for (key, batches) in self.datasets.read().iter() {
            ...
            let message: arrow_flight::IpcMessage =
                arrow_flight::SchemaAsIpc::new(first.schema().as_ref(), &options)
                    .try_into()
            ...
            infos.push(Ok(FlightInfo {
                ...
                endpoint: vec![FlightEndpoint::new().with_ticket(Ticket::new(key.clone()))],
                total_records: batches.iter().map(|b| b.num_rows() as i64).sum(),
                total_bytes: -1,
                ordered: false,
    ...
    async fn get_flight_info(
        ...
        let schema = batches
            .first()
            .ok_or_else(|| Status::not_found("dataset empty"))?
            .schema();
        let options = arrow_ipc::writer::IpcWriteOptions::default();
        let message: arrow_flight::IpcMessage =
            arrow_flight::SchemaAsIpc::new(schema.as_ref(), &options)
                .try_into()
                .map_err(|e: arrow::error::ArrowError| Status::internal(e.to_string()))?;
    ...
    async fn poll_flight_info(
        ...
        Err(Status::unimplemented(
            "GraphVis jobs use DoExchange for streaming computation",
        ))
    }
    ...
    async fn get_schema(
        ...
        let options = arrow_ipc::writer::IpcWriteOptions::default();
        let message: arrow_flight::IpcMessage =
            arrow_flight::SchemaAsIpc::new(batch.schema().as_ref(), &options)
                .try_into()
                .map_err(|e: arrow::error::ArrowError| Status::internal(e.to_string()))?;
        let bytes = message.0;
    ...
    async fn do_get(
        ...
        let batches = self
            ...
            .get(&key)
            .cloned()
            .ok_or_else(|| Status::not_found("ticket not found"))?;
    ...
    async fn do_put(
        ...
        let first = incoming
            .next()
            .await
            .ok_or_else(|| Status::invalid_argument("empty DoPut"))??;
        let key = first
            .flight_descriptor
            .as_ref()
            .map(Self::key_from_descriptor)
        ...
        while let Some(batch) = decoded.next().await {
            batches.push(batch.map_err(|e| Status::invalid_argument(e.to_string()))?);
        }
    ...
    async fn do_action(
        ...
        if action.r#type == "graphvis.sql" {
            let query = String::from_utf8(action.body.to_vec())
                .map_err(|_| Status::invalid_argument("SQL action body must be UTF-8"))?;
            ...
            for (name, batches) in self.datasets.read().iter() {
                if let Some(first) = batches.first() {
                    ...
                    let safe = name
                        ...
                        .collect::<String>();
                    ctx.register_table(&safe, Arc::new(table))
                        .map_err(|e| Status::internal(e.to_string()))?;
                }
            }
            let batches = ctx
                ...
                .await
                .map_err(|e| Status::internal(e.to_string()))?;
            let ticket = format!("result:{}", Uuid::new_v4());
            self.datasets.write().insert(ticket.clone(), batches);
            return Ok(Response::new(Box::pin(stream::iter(vec![Ok(
                arrow_flight::Result {
                    body: ticket.into(),
                },
            )]))));
    ...
    async fn list_actions(
        ...
        Ok(Response::new(Box::pin(stream::iter(vec![
            Ok(ActionType {
                r#type: "graphvis.capabilities".into(),
                description: "Return GraphVis worker capabilities".into(),
            }),
            Ok(ActionType {
                r#type: "graphvis.sql".into(),
                description: "Execute SQL and return a Flight ticket for the Arrow result".into(),
            }),
        ]))))
    }
