...
impl Default for CameraState {
    fn default() -> Self { Self { azimuth_deg: -40.0, elevation_deg: 25.0, pan_x: 0.0, pan_y: 0.0, zoom: 1.0 } }
}
...
impl NativeSurfaceRenderer {
    ...
    fn from_surface(instance: wgpu::Instance, surface: wgpu::Surface<'static>, width: u32, height: u32) -> Result<Self> {
        ...
        let mut config = surface.get_default_config(&adapter, width.max(2), height.max(2)).context("surface has no default configuration")?;
        ...
        let layout = device.create_pipeline_layout(&wgpu::PipelineLayoutDescriptor {
            label: Some("GraphVis native pipeline layout"), bind_group_layouts: &[Some(&bind_layout)], immediate_size: 0,
        });
        const ATTRS: [wgpu::VertexAttribute; 4] = wgpu::vertex_attr_array![0=>Float32x3, 1=>Float32, 2=>Float32, 3=>Float32];
        let pipeline = device.create_render_pipeline(&wgpu::RenderPipelineDescriptor {
            ...
            vertex: wgpu::VertexState {
                ...
                buffers: &[Some(wgpu::VertexBufferLayout { array_stride: std::mem::size_of::<PointVertex>() as u64, step_mode: wgpu::VertexStepMode::Instance, attributes: &ATTRS })],
            },
        ...
        let camera_uniform = CameraUniform { view_proj: Mat4::IDENTITY.to_cols_array_2d(), view: [width as f32, height as f32, 1.0, 0.85], style: [0.0, 0.05, 0.0, 0.0] };
        let camera_buffer = device.create_buffer_init(&wgpu::util::BufferInitDescriptor {
            label: Some("GraphVis persistent camera uniform"), contents: bytemuck::bytes_of(&camera_uniform), usage: wgpu::BufferUsages::UNIFORM | wgpu::BufferUsages::COPY_DST,
        });
        let camera_bind_group = device.create_bind_group(&wgpu::BindGroupDescriptor { label: Some("GraphVis persistent camera bind"), layout: &bind_layout, entries: &[wgpu::BindGroupEntry { binding:0, resource:camera_buffer.as_entire_binding() }] });
        let (depth_texture, depth_view) = Self::create_depth(&device, width.max(2), height.max(2));
        Ok(Self { _instance: instance, surface, device, queue, config, pipeline, camera_buffer, camera_bind_group, depth_texture, depth_view, point_buffer: None, point_capacity_bytes: 0, point_count: 0, camera: CameraState::default(), point_scale: 5.0, alpha_scale: 0.85, invert_opacity: false, min_alpha: 0.05, colormap_id: 0.0, clear: wgpu::Color { r:0.027, g:0.035, b:0.047, a:1.0 } })
    }

    fn create_depth(device: &wgpu::Device, width: u32, height: u32) -> (wgpu::Texture, wgpu::TextureView) {
        let texture = device.create_texture(&wgpu::TextureDescriptor {
            label: Some("GraphVis depth buffer"), size: wgpu::Extent3d { width, height, depth_or_array_layers: 1 }, mip_level_count: 1, sample_count: 1, dimension: wgpu::TextureDimension::D2,
    ...
    pub fn resize(&mut self, width: u32, height: u32) {
        ...
        self.depth_texture = texture; self.depth_view = view;
    }

    pub fn set_camera(&mut self, camera: CameraState) { self.camera = camera; }
    pub fn set_style(&mut self, point_scale: f32, alpha_scale: f32, invert_opacity: bool) {
        self.point_scale = point_scale.max(1.0); self.alpha_scale = alpha_scale.clamp(0.0, 1.0); self.invert_opacity = invert_opacity;
    }
    /// Apply a Smart Render style without rebuilding geometry or GPU pipeline objects.
    pub fn set_smart_style(&mut self, point_scale:f32, alpha_scale:f32, invert_opacity:bool, min_alpha:f32, colormap_id:u32) {
    ...
    pub fn set_points(&mut self, points: &[PointVertex]) {
        ...
        if self.point_buffer.is_none() || required > self.point_capacity_bytes {
            ...
            self.point_buffer = Some(self.device.create_buffer(&wgpu::BufferDescriptor {
                label: Some("GraphVis persistent point buffer"), size: capacity, usage: wgpu::BufferUsages::VERTEX | wgpu::BufferUsages::COPY_DST, mapped_at_creation: false,
            }));
            self.point_capacity_bytes = capacity;
        }
        if let Some(buffer) = &self.point_buffer { self.queue.write_buffer(buffer, 0, bytes); }
    }
    ...
    pub fn render(&mut self) -> Result<()> {
        let (frame, reconfigure_after_present) = match self.surface.get_current_texture() {
            ...
            wgpu::CurrentSurfaceTexture::Outdated | wgpu::CurrentSurfaceTexture::Lost => {
                ...
                match self.surface.get_current_texture() {
                    ...
                    wgpu::CurrentSurfaceTexture::Suboptimal(frame) => (frame, true),
                    wgpu::CurrentSurfaceTexture::Timeout | wgpu::CurrentSurfaceTexture::Occluded => return Ok(()),
                    wgpu::CurrentSurfaceTexture::Outdated => return Err(anyhow!("surface remained outdated after reconfiguration")),
                    wgpu::CurrentSurfaceTexture::Lost => return Err(anyhow!("surface was lost after reconfiguration")),
                    wgpu::CurrentSurfaceTexture::Validation => return Err(anyhow!("surface acquisition validation error")),
                }
            }
            wgpu::CurrentSurfaceTexture::Validation => return Err(anyhow!("surface acquisition validation error")),
        };
        let view = frame.texture.create_view(&Default::default());
        let rotation = Mat4::from_rotation_x(self.camera.elevation_deg.to_radians()) * Mat4::from_rotation_z(self.camera.azimuth_deg.to_radians());
        let aspect = self.config.width as f32 / self.config.height.max(1) as f32;
        let proj = Mat4::perspective_rh_gl(45f32.to_radians(), aspect, 0.01, 100.0);
        let eye = Vec3::new(0.0, -3.2 / self.camera.zoom.clamp(0.1, 12.0), 1.8 / self.camera.zoom.clamp(0.1, 12.0));
        ...
        let uniform = CameraUniform {
            ...
            view: [self.config.width as f32, self.config.height as f32, self.point_scale, self.alpha_scale],
            style: [if self.invert_opacity { 1.0 } else { 0.0 }, self.min_alpha, self.colormap_id, 0.0],
        };
        ...
        let mut encoder = self.device.create_command_encoder(&Default::default());
        {
            let mut pass = encoder.begin_render_pass(&wgpu::RenderPassDescriptor {
                ...
                color_attachments: &[Some(wgpu::RenderPassColorAttachment { view:&view, resolve_target:None, ops:wgpu::Operations { load:wgpu::LoadOp::Clear(self.clear), store:wgpu::StoreOp::Store }, depth_slice:None })],
                depth_stencil_attachment: Some(wgpu::RenderPassDepthStencilAttachment {
                    view: &self.depth_view, depth_ops: Some(wgpu::Operations { load: wgpu::LoadOp::Clear(1.0), store: wgpu::StoreOp::Store }), stencil_ops: None,
                }), timestamp_writes: None, occlusion_query_set: None, multiview_mask: None,
            });
            if let Some(points) = &self.point_buffer {
                pass.set_pipeline(&self.pipeline); pass.set_bind_group(0, &self.camera_bind_group, &[]); pass.set_vertex_buffer(0, points.slice(..)); pass.draw(0..6, 0..self.point_count);
            }
        }
...
#[derive(Debug, Clone, Copy)]
pub struct VoxelCell { pub center:[f64;3], pub mean:f64, pub count:u32 }
...
pub fn voxel_aggregate(x:&[f64], y:&[f64], z:&[f64], response:&[f64], bins:[usize;3]) -> Vec<VoxelCell> {
    ...
    let accum=(0..n).into_par_iter().fold(|| vec![(0.0f64,0u32);total], |mut a,i| {
        let vals=[x[i],y[i],z[i],response[i]]; if vals.iter().all(|v|v.is_finite()) {
            let ix=(((x[i]-xl)/(xh-xl).max(f64::EPSILON))*bins[0] as f64).floor().clamp(0.0,(bins[0]-1) as f64) as usize;
            let iy=(((y[i]-yl)/(yh-yl).max(f64::EPSILON))*bins[1] as f64).floor().clamp(0.0,(bins[1]-1) as f64) as usize;
            let iz=(((z[i]-zl)/(zh-zl).max(f64::EPSILON))*bins[2] as f64).floor().clamp(0.0,(bins[2]-1) as f64) as usize;
            let k=ix + bins[0]*(iy + bins[1]*iz); a[k].0 += response[i]; a[k].1 += 1;
        } a
    }).reduce(|| vec![(0.0,0);total], |mut a,b| { for(i,(s,c)) in b.into_iter().enumerate(){a[i].0+=s;a[i].1+=c;} a });
