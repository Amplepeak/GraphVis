...
QuadFit fitQuadratic(const QVector<double>& x,const QVector<double>& y){
    QuadFit fit;
...
PlotSpec QtPlotBackend::prepareSpecCore(const PlotSpec& in) const {
    ...
    if(in.engine==QLatin1String("IDF Curve")){
        ...
        if(in.series.size()>=3){
            ...
            for(auto it=byPeriod.constBegin();it!=byPeriod.constEnd();++it,++index){
                ...
                QVector<double> lx,ly;
                PlotSeries s;
                s.color=QColor::fromHsvF(std::fmod(0.58+double(index)*0.13,1.0),0.62,0.95);
    ...
    if(in.engine==QLatin1String("Tauc Plot")){
        ...
        if(in.series.size()>=2){
            ...
            if(rows.size()>=6){
                ...
                int bestAt=-1; double bestSlope=0.0; LineFit best;
                for(int start=0;start+window<=rows.size();++start){
                    QVector<double> wx=xs.mid(start,window),wy=ys.mid(start,window);
                    const LineFit f=fitLine(wx,wy);
                    if(!f.ok||!(f.slope>bestSlope)) continue;
                    bestSlope=f.slope; best=f; bestAt=start;
                }
    ...
    if(in.engine==QLatin1String("Creep Curve")){
        ...
        if(in.series.size()>=2){
            ...
            if(rows.size()>=6){
                ...
                LineFit best; int bestAt=-1; double bestSlope=0.0;
                for(int start=0;start+window<=rows.size();++start){
                    QVector<double> wx=xs.mid(start,window),wy=ys.mid(start,window);
                    const LineFit f=fitLine(wx,wy);
                    if(!f.ok) continue;
                    if(bestAt<0||f.slope<bestSlope){ best=f; bestSlope=f.slope; bestAt=start; }
                }
                if(bestAt>=0&&best.ok){
        ...
        if(in.series.size()>=3){
            ...
            if(isotherms.size()>=1){
                ...
                for(double t:temperatures){
                    ...
                    s.color=QColor::fromHsvF(std::fmod(0.02+double(index)*0.13,1.0),0.6,0.95);
    ...
    if(in.engine==QLatin1String("Yield Curve")){
        ...
        if(in.series.size()>=3){
            ...
            for(auto it=byDate.constBegin();it!=byDate.constEnd();++it,++index){
                ...
                if(rows.isEmpty()) continue;
                PlotSeries s;
                s.color=QColor::fromHsvF(std::fmod(0.33+double(index)*0.11,1.0),0.6,0.95);
                s.lineWidth=qMax(1.3,in.style.lineWidth);
                s.drawMarkers=true; s.markerSize=3.4;
        ...
        if(in.series.size()>=3){
            ...
            const auto composite=[](const QVector<Stream>& streams){
                ...
                for(int i=1;i<levels.size();++i){
                    ...
                    for(const Stream& s:streams)
                        if(s.lo<=levels[i-1]&&s.hi>=levels[i]) cp+=s.cp;
            ...
            if(hotCurve.size()>=2&&coldCurve.size()>=2){
                const auto temperatureAt=[](const QVector<QPair<double,double>>& c,double h){
                    ...
                    int hi=1;
                    while(hi<c.size()&&c[hi].first<h) ++hi;
    ...
    if(in.engine==QLatin1String("Harmonic Spectrum")){
        ...
        if(in.series.size()>=2){
            ...
            if(!lines.x.isEmpty()){
                lines.label=(std::abs(fundamental)>1e-300)
                    ? QStringLiteral("THD %1%").arg(100.0*std::sqrt(harmonics)/std::abs(fundamental),0,'f',2)
                    : QStringLiteral("no order-1 component - THD undefined");
    ...
    if(in.engine==QLatin1String("Group Delay")){
        ...
        if(in.series.size()>=2){
            ...
            if(rows.size()>=3){
                ...
                if(used>0){
                    delay.label=QStringLiteral("mean %1, ripple %2 peak to peak")
                                    .arg(sum/double(used),0,'g',4)
                                    .arg(largest-smallest,0,'g',4);
